<template>
      ...
      <BreezeAuthenticatedLayout>
        <template #header>
          <div class="grid grid-cols-2 grid-flow-col">
            ...
            <Link href="/fav-quotes" :only="['quotes']">
              <RefreshCw />
            </Link>
          </div>
        </template>
          ...
          <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
              <div class="p-6 bg-white border-b border-gray-200">
                <div v-for="quote in quotes.data" class="grid grid-cols-2 grid-flow-col p-2 rounded border my-2">
                  <div class="col-span-2">{{ quote.quote }} - {{ quote.author }}</div>
                  <div class="cursor-pointer">
                    <Trash2 @click="deleteQuote(quote.id)"/>
                  </div>
                </div>
  ...
  async function deleteQuote(quoteId: number) {
    await axios.delete(route('fav-quotes'), {
      data: {
        id: quoteId,
      },
    });
